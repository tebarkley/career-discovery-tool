...
master_df = pd.io.pickle.read_pickle(path.join(pardir, pardir, 'data', 'anna_jobs_Feb9.pkl'))

#Get the job descriptions that we've already called
if path.isfile(path.join(pardir, pardir, 'data', 'anna_job_descriptions.pkl')):
    obtained_jobs = pd.io.pickle.read_pickle(path.join(pardir, pardir, 'data', 'anna_job_descriptions.pkl'))
...
if path.isfile(path.join(pardir, pardir, 'data', 'anna_invalid_job_ids.pkl')):
    invalid = set(pd.io.pickle.read_pickle(path.join(pardir, pardir, 'data', 'anna_invalid_job_ids.pkl')))
else:
    invalid = set()
...
all_jobs.to_pickle(path.join(pardir, pardir, 'data', 'anna_job_descriptions.pkl'))